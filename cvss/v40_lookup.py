"""Tabla de busqueda oficial MacroVector -> score de CVSS v4.0.

ARTEFACTO DE REFERENCIA CONGELADO. Transcripcion verbatim de las entradas
MacroVector->score de la tabla oficial de FIRST: claves y valores reproducidos
sin modificacion (el contenedor se paso de JS a un dict de Python, los datos no).

Origen  : FIRSTdotorg/cvss-v4-calculator (cvss_lookup.js). La misma tabla esta
          vendorizada como CVSS_LOOKUP_GLOBAL en RedHatProductSecurity/cvss.
Pin     : RedHatProductSecurity/cvss v3.6, commit 2f149099257ae06b98cef252efc440bddafe61e5 ->
          cvss/constants4.py -> CVSS_LOOKUP_GLOBAL (identico a esta tabla).
Fetch   : tomado de FIRSTdotorg/cvss-v4-calculator `main` el 2026-08-31.
Verif.  : seal_upstream.py ejecuta cvss==3.6 (v3.6) real sin modificar y da 0
          divergencias sobre 20.087 vectores => esta tabla corresponde a v3.6.
Licencia: BSD-2-Clause (Copyright FIRST, Red Hat, and contributors)
Integridad verificada por test: 270 MacroVectors, claves de 6 digitos, scores en
[0,10], y la restriccion conjunta EQ3=2 -> EQ6=1.
"""
